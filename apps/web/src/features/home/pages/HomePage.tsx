...
import { fetchMenuTypes, fetchSiteSettings, publicUploadUrl } from '@/shared/lib/api';
        ...
        <div className="mx-auto max-w-2xl animate-in px-4 pb-10 pt-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="overflow-hidden rounded-2xl border border-white/5 bg-app-panel/30"
              >
      ...
      <div className="mx-auto max-w-2xl animate-in px-4 pb-10 pt-6 sm:px-5">
        <header className="mb-8 text-center sm:mb-10">
          ...
          <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-stone-400">
            Откройте нужное меню, добавляйте блюда в корзину через иконку в шапке, затем
            оформите заказ.
          </p>
        </header>
                ...
                >
                  {img ? (
                    ...
                    <div className="flex aspect-5/3 w-full items-center justify-center bg-linear-to-br from-stone-800/90 to-stone-950">
                      <UtensilsCrossed
                        className="h-14 w-14 text-app-accent/22"
                        strokeWidth={1.1}
                        aria-hidden
                      />
                    </div>
                  ...
                  <div className="flex items-center justify-between gap-3 border-t border-white/6 bg-black/15 px-4 py-4">
                    ...
                    <ChevronRight
                      className="h-5 w-5 shrink-0 text-app-accent/55 transition group-hover:translate-x-0.5 group-hover:text-app-accent"
                      strokeWidth={2}
                      aria-hidden
                    />
                  </div>

...
const MAIN_MENU_CATEGORIES: CategorySeed[] = [
  {
...
const BAR_MENU_CATEGORIES: CategorySeed[] = [
  {
    nameRu: 'Коктейли',
    nameEn: 'Cocktails',
    sortOrder: 0,
                ...
                create: cat.items.map((item, idx) => ({
                  ...
                  translations: {
                    create: [
                      ...
                      ...(item.nameEn
                        ? [{ locale: 'en' as const, name: item.nameEn, description: (item.descriptionEn ?? item.descriptionRu) ?? null }]
                        : [{ locale: 'en' as const, name: item.nameRu, description: (item.descriptionEn ?? item.descriptionRu) ?? null }]),
                    ],
                  },
                })),
              }
